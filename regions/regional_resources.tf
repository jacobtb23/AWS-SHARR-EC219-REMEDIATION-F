...
resource "aws_lambda_function" "SG_Remediation_Lambda" {
  function_name    = "ASR-REMEDIATE-SGs-EC2-19"
  role             = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/SO0111-Remediate-SC-2.0.0-EC2.19"
...
resource "aws_lambda_permission" "lambda_trust_sharr" {
  statement_id  = "LambdaTrustSHARRExecutionRole"
  action        = "lambda:InvokeFunction"
  function_name = "ASR-REMEDIATE-SGs-EC2-19"
  principal     = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/SO0111-Remediate-SC-2.0.0-EC2.19"
  source_arn    = "arn:aws:lambda::${data.aws_caller_identity.current.account_id}:function:ASR-REMEDIATE-SGs-EC2-19"
}
...
resource "aws_ssm_document" "ssm_automation" {
  name            = "ASR-SC_2.0.0_EC2.19"
...
schemaVersion: "0.3"
description: >
  ### Document Name - ASR-SC_2.0.0_EC2.19
...
parameters:
  ...
  RemediationRoleName:
    default: SO0111-Remediate-SC-2.0.0-EC2.19
    type: String
    allowedPattern: ^[\w+=,.@-]+$
  Finding:
...
mainSteps:
      ...
      Script: >
        ...
        class FindingEvent:
            ...
            def __init__(self, finding_json, parse_id_pattern, expected_control_id, resource_index):
                ...
                self.affected_object = {'Type': self.resource['Type'], 'Id': self.resource_id}
  ...
  - inputs:
      FunctionName: ASR-REMEDIATE-SGs-EC2-19
      InputPayload:
        action: InvokeLambdaFromSSM
        Name: EC2.19 Remediation Document
        GroupId:
          - "{{ParseInput.GroupId}}"
        RemediationAccount:
